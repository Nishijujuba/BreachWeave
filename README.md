...
Observer 持续检查最近几轮执行轨迹和反馈；发现明显低效或偏移时，只做轻量纠偏，不替代 Solver 做决定。
...
任务是否结束，由系统结合任务状态统一约束，模型主观判断只作辅助，避免复杂任务在中途被过早结束。

### 5. 上下文压缩与降噪

系统通过改写、压缩、摘要等方式控制原始工具输出和历史会话体积，尽量让后续决策始终建立在高信号、低噪音的信息之上。
